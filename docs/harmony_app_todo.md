...
所有标记在 `agents/device/harmony.py` 内联（`NOTE/verify-on-device`）。

| 方法 | 待核 |
| --- | --- |
| `screencap` | `uitest screenCap -p <path>` 路径形态 + `hdc file recv` 行为 |
| `screen_size` | `hidumper -s RenderService` 哪个 section 带物理分辨率；解析正则 |
| `dump_ui_tree` / `_layout_to_nodes` / `_parse_bounds` | `uitest dumpLayout` JSON 的字段名（`id` vs `key`、`description` 等）、`bounds` 形态与 flag 取值 |
| `foreground_app` / `_parse_foreground_bundle` | `aa dump -l` 前台 bundle 的确切行格式；`_AA_BUNDLE_RE` 正则 |
| `swipe_gesture` | `uiInput swipe` velocity(px/s) 换算系数（`_SWIPE_VELOCITY_MIN/MAX`）是否合手感 |
| `key` ENTER | `uiInput keyEvent` 的 Enter token（暂用 keycode `2054`，`_HDC_KEYS`）|
| `input_text` | `uiInput inputText <x> <y> <text>` 复用 `_last_tap` 坐标的正确性；CJK/空格/引号经单 argv 的 round-trip 保真 |
| `kill_all_apps` | `aa dump -l` 枚举活动 mission 的格式 + `_PROTECTED_BUNDLES` 是否够全 |
| `launch` | `aa start -b <bundle> -a <ability>` 默认 ability（`EntryAbility`）是否通用 |
| `start_recording` | HarmonyOS 端上视频录屏（`uitest uiRecord` 只录动作）—— 暂 `_NoRecording` no-op |

> 改法：命令/解析都隔离在小函数（`_run`/`_recv`/`_ui_input`/`_parse_*`/`_split_app_id`），真机观察后单点修正即可。
