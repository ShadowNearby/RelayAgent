...
| `tap` / `long_press` | `uitest uiInput click / longClick`（tap 记 `_last_tap`）|
| `swipe_gesture` | `uitest uiInput swipe`（duration_ms → velocity px/s 换算）|
| `key` | `uitest uiInput keyEvent Back / Home / 2054`（`_HDC_KEYS`）|
| `input_text` | `uitest uiInput inputText <x> <y> <text>`（复用最近 `_last_tap` 坐标；无 IME，CJK 直发）|
| `launch` / `force_stop` | `aa start -b <bundle> -a <ability>`（`_split_app_id`，默认 `EntryAbility`）/ `aa force-stop <bundle>` |
| `cold_launch` | force_stop + launch + settle（照搬 android）|
| `kill_all_apps` | `aa dump -l` 枚举活动 mission（`_AA_BUNDLE_RE`）∖ `_PROTECTED_BUNDLES` → force-stop + HOME |
| `setup/teardown_input_channel` | no-op 返回 True（uiInput 不需 IME swap）|
| `start_recording` | 返回 `_NoRecording` 句柄 + loud warning（HarmonyOS shell 无 screenrecord 等价）|
| `dismiss_permission_popup` | 复用 `vendor_profiles` 的 Allow 标签策略（弹窗在 dumpLayout 内）|

实现要点：
- `hdc_base()` / `_run(args, timeout, text)` 与 `android._run` 同构的 subprocess 封装；`_recv` 封 `hdc file recv`。
- `_layout_to_nodes(json)` 纯函数：递归 walk dumpLayout 的 `{attributes, children}` 树，文档序产
  `UINode[]`；bounds 经 `_parse_bounds`（复用 `[x1,y1][x2,y2]` 正则）。可无设备单测。
- `_HDC_KEYS = {BACK:"Back", HOME:"Home", ENTER:"2054"}`；swipe 用 `_SWIPE_VELOCITY_MIN/MAX` clamp。
- `_split_app_id` 拆 `bundle[/ability]`；`_NoRecording` 是 no-op RecordingHandle。
...
**单测**（无设备）：harmony 用例在 [`tests/test_device_backend.py`](../tests/test_device_backend.py) 的
`HarmonyArgvTests` / `LayoutToNodesTests` — mock `agents.device.harmony.subprocess.run`，钉 argv 构造
（`-t` 注入）、JSON→UINode 解析、`screen_size` 缓存、factory dispatch 等。
...
- ✅ host `harmony.py` 实现 + 单测（`test_device_backend.py` 的 `HarmonyArgvTests`/`LayoutToNodesTests`）；factory 分发。
