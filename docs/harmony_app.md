...
| `foreground_app` | `aa dump -l` → `_parse_foreground_bundle` |
| `tap` / `long_press` | `uitest uiInput click / longClick` (tap records `_last_tap`) |
| `swipe_gesture` | `uitest uiInput swipe` (duration_ms → velocity px/s conversion) |
| `key` | `uitest uiInput keyEvent Back / Home / 2054` (`_HDC_KEYS`) |
| `input_text` | `uitest uiInput inputText <x> <y> <text>` (reuses the latest `_last_tap`; no IME, CJK direct) |
| `launch` / `force_stop` | `aa start -b <bundle> -a <ability>` (`_split_app_id`, default `EntryAbility`) / `aa force-stop <bundle>` |
| `cold_launch` | force_stop + launch + settle (same as android) |
| `kill_all_apps` | `aa dump -l` live missions (`_AA_BUNDLE_RE`) ∖ `_PROTECTED_BUNDLES` → force-stop + HOME |
| `setup/teardown_input_channel` | no-op returning True (uiInput needs no IME swap) |
| `start_recording` | returns a `_NoRecording` handle + loud warning (no screenrecord equivalent on HarmonyOS shell) |
...
- `hdc_base()` / `_run(args, timeout, text)` is the same subprocess wrapper shape as `android._run`;
  `_recv` wraps `hdc file recv`.
- `_layout_to_nodes(json)` is a pure function: recursively walks the dumpLayout `{attributes,
  children}` tree producing `UINode[]` in document order; bounds via `_parse_bounds` (reusing the
  `[x1,y1][x2,y2]` regex). Unit-testable without a device.
- `_HDC_KEYS = {BACK:"Back", HOME:"Home", ENTER:"2054"}`; swipe clamps to `_SWIPE_VELOCITY_MIN/MAX`.
- `_split_app_id` splits `bundle[/ability]`; `_NoRecording` is a no-op RecordingHandle.
...
**Tests** (device-less): harmony cases live in [`tests/test_device_backend.py`](../tests/test_device_backend.py)
(`HarmonyArgvTests` / `LayoutToNodesTests`) — they mock `agents.device.harmony.subprocess.run` and pin
argv construction (`-t` injection), JSON→UINode parsing, `screen_size` caching, factory dispatch, etc.
...
- ✅ host `harmony.py` implemented + unit tests (`HarmonyArgvTests`/`LayoutToNodesTests` in
  `test_device_backend.py`); factory dispatch.
