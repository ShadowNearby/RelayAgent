...
**Switches / knobs**: `RELAY_MW_FALLBACK` (default `1`; `0` or `run_plan.py --no-mw-fallback` disables it) / `RELAY_MW_SERVER_URL` (default `http://127.0.0.1:6800`) / `RELAY_MW_MAX_ROUND` (default 25) / `RELAY_MW_TIMEOUT` (default 600). The preview marks MW legs with `[MobileWorld fallback]`.

### 10.1 General fallback (when MobileWorld is unavailable)

MW fallback depends on the host-side MobileWorld runtime (optional extra `mw` + a server) — the on-device app doesn't have it, and most open-source users won't install it. So with MW **off** (`RELAY_MW_FALLBACK=0`, the on-device default) and `RELAY_GENERAL_FALLBACK=1` (default on), every trigger above converts to a `type: general` leg instead, executed by **this repo's own manifest-free general GUI agent** (`agents/agent/general_agent.py:GeneralGUIAgent` — a11y-tree step-by-step driving, inheriting the A11yTextAgent loop/serialization/CTA safety rails) on the **same native runtime** — no external process; on-device it runs in-process. Priority is strictly **MW > general > unsatisfiable**, so default host behavior has zero drift.

- **Leg shape**: same contract as an MW leg (keeps `id`/`prompt`/`bind`/`extract`; `app` is only a launch hint; without one, `RELAY_TARGET_APP=__home__` makes the agent press HOME and find an app itself). Validation/routing skips reuse the MW-leg path (`_is_fallback_leg`).
- **Execution** (`FlowRunner._run_general_step`): runs `native_runner` through the LegExecutor with `RELAY_AGENT_FILE` pointing at the general agent (packaged runtimes fall back to the module spec); a `finish` action may carry an `answer` text → fed back to the blackboard via `RELAY_REPLY_OUT`; the leg judge runs as usual. Step cap `RELAY_GENERAL_MAX_STEP` (default 25, mirroring `RELAY_MW_MAX_ROUND`).
- **Recovery ladder**: with MW off, the ladder's last tier likewise becomes general (`recovered_via: general_fallback`); the handoff red line is unchanged (retry tier only).
- **Switches**: `RELAY_GENERAL_FALLBACK` (default `1`; `0` or `run_plan.py --no-general-fallback` disables it — only with both fallbacks off does the old unsatisfiable exit return). `run_benchmark_test.py` unconditionally forces `0` (the eval's fallback axis is MW).
- **Safety**: the general agent never crosses an irreversible CTA (pay/submit labels convert to an ask_user handoff), same list as the a11y baseline.
