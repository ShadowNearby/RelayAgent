...
RelayAgent no longer depends on MobileWorld's `mw test`; the single-app entry is
now `scripts/run_native.py`, which drives the same agent over direct adb (no
uvicorn server). This shim preserves the old invocation so existing muscle
