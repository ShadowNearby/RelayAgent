...
def main(argv: list[str] | None = None) -> int:
    ...
    # The flow traj root is only known once FlowRunner is constructed; capture
    # it so the finally below can harvest each leg's token log into the report.
    ...
    try:
        ...
        try:
            ...
            flow_traj_root = runner.flow_traj_root
            runner.run()
            completed = True
            return 0
        finally:
