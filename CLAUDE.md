...
- **执行期失败恢复（默认开）**：app leg 失败先分类（`env_fail` 不恢复 / `route_fail` / `app_fail`），再爬梯子：重试（route_fail 先 LLM 换措辞，模板能力不改措辞）→ 换路由（router `exclude` 失败对）→ 运行期 MW 兜底 → 部分成功报告（flow 根 `flow_report.json`，原 leg 旁 `recovery.json`）。handoff 能力只允许重试档（不换路由、不进 MW）。旋钮 `RELAY_RECOVERY`(1)/`RELAY_RECOVERY_MAX_RETRIES`(1)/`RELAY_RECOVERY_MAX_LEGS`(2)/`RELAY_RECOVERY_TOKEN_BUDGET`(15000)；`run_benchmark_test.py` 默认强制关，`--recovery` 开——开了后每行 results 落 `recovery` 遥测块（harvest 自 flow 根 `flow_report.json`，attempts 带逐次 token 成本），`summary.json`/`summary.md` 出首试 vs 最终成功 + 逐档命中率 + 恢复 token 通胀（R4，`tests/test_benchmark_recovery.py` 钉住）。详见 nl_flow §6.1，实现 `agents/flow/leg_recovery.py`。
- **capability 不覆盖 → MobileWorld 兜底**：任何 unsatisfiable（coverage gap 修复用尽 / LLM 判整条不可满足 / **repair 用尽仍 invalid** / **stage-3 判设备动作非 foundation 任务**）不再放弃，而是把那条 leg（或整条请求）转成 `type: mobileworld` leg，交给 MobileWorld 无 manifest 的 `general_e2e` 执行（经 `scripts/run_mobileworld.py`，answer 文本回灌 blackboard）。默认开，`RELAY_MW_FALLBACK=0` / `run_plan.py --no-mw-fallback` 关；`RELAY_MW_MAX_ROUND`(25)/`RELAY_MW_TIMEOUT`(600)。详见 nl_flow §10。
  - **MW 不可用时 → 通用兜底（general fallback）**：MW 关闭（端侧默认 / 主机没装 `mw` extra）且 `RELAY_GENERAL_FALLBACK=1`（默认）时，同样的触发点转成 `type: general` leg，由本仓库无 manifest 的通用 GUI agent（`agents/agent/general_agent.py:GeneralGUIAgent`，A11yTextAgent 子类：a11y 树逐步驱动 + CTA 安全护栏 + `finish.answer` 回灌 blackboard）在同一 native runtime 上执行（`FlowRunner._run_general_step` 经 LegExecutor；无 app 提示时 `RELAY_TARGET_APP=__home__` 从 HOME 自行找 app）。恢复梯子最后一档同理。优先级 **MW > general > unsatisfiable**（主机默认零漂移）；`RELAY_GENERAL_MAX_STEP`(25) 限步；`run_plan.py --no-general-fallback` 关；`run_benchmark_test.py` 无条件强制关。详见 nl_flow §10.1。
...
- **App 界面**（Material 3 + viewBinding，见 `android/README.md` §App 界面）：主页为**会话式任务流**（对标 Codex/Claude App，2026-07 改版）——任务=右侧气泡，运行=实时活动卡（子任务行 ▸/✓ + 当前步骤，`RunEvents.kt` 类型化事件总线喂），结果=结果卡（成败 + 回复 + 跳运行详情），底部胶囊输入栏（运行中变停止），空线程显示问候 + 示例建议；线程存内存单例 `ChatStore`（`ChatThread.kt`）。`ExamplesActivity`（读 `res/raw/examples.json`，50 条由 `scripts/android/gen_app_examples.py` 从 `benchmark/` 生成，改基准后重跑刷新）、结构化运行日志查看器（`LogActivity` 运行列表 → `RunDetailActivity` 子任务 → `LegDetailActivity` 步骤时间线含截图缩略图；解析 `TrajLog.kt`，App 名 `AppLabels.kt`，原始文件树退到 `RawLogActivity`+`LogDetailActivity`；`entry.py` 落 `meta.json` 存任务原文）。`OverlayController.postStatus` 把 `emit_status` 事件同源喂 chip / `RunLog` / `RunEvents`。
